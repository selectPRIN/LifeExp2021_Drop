...
##AUSTRALIA is coded "AUS2" in STMF and "AUS" in HMD
...
mymatrix <- data.frame("Country"=NA,"Year"=NA,"e0.smtf"=NA,"e0.hmd"=NA,"Week"=NA,"ratio014"=NA,"ratio1564"=NA,"ratio6574"=NA,"ratio7584"=NA,"ratio85p"=NA)
...
for (i in 1:length(names(table(stmf$CountryCode)))){
    ...
    cnt.lt <- hmd.lt(country=cnt,user="USER",password="PWD")
    ...
    for(y in as.numeric(names((table(stmf$Year[stmf$CountryCode == cnt]))))[1]:2019){
        ...
        ratios <- m.hmd/mx.y
    line <- c(cnt,y,e0.s,e0.h,52,ratios[1],ratios[2],ratios[3],ratios[4],ratios[5],ratios[6])
    mymatrix <- rbind(mymatrix,line)
    }
    #Compute 2020 life expectancy:
    ...
    #Compute 2021 life expectancy:
    # 1) mortality rates of future weeks are the same as those recorded on the same weeks in 2019
    Av.Weeks.21 <- max(as.numeric(names(table(stmf$Week[stmf$Country==cnt&stmf$Year==2021]))))
    if(is.infinite(Av.Weeks.21)){Av.Weeks.21  <-  1}
    D.y.21 <- sum(stmf[which(stmf$CountryCode==cnt&stmf$Sex=="b"&stmf$Year==2021),10],na.rm=TRUE)
    D.nodate.21 <- input$Deaths[which(input$Sex=="b"&input$Year==2021&input$Week=="UNK"&input$Age=="TOT")]
    D.nodate.21 <- ifelse(length(D.nodate.21)==0,0,D.nodate.21)
    ...
    mx.2021 <- ((D.nodate.21+D.y.21)/D.y.21)*colSums(stmf[which(stmf$CountryCode==cnt&stmf$Sex=="b"&stmf$Year==2021&stmf$Week<=Av.Weeks.21),11:15])/max(Av.Weeks.21,1)
    mx.2019 <- ((D.nodate.19+D.y.19)/D.y.19)*colSums(stmf[which(stmf$CountryCode==cnt&stmf$Sex=="b"&stmf$Year==Av.L.Y&stmf$Week>Av.Weeks.21),11:15])/max((52-Av.Weeks.21),1)
    mx2019 <- ((D.nodate.19+D.y.19)/D.y.19)*colSums(stmf[which(stmf$CountryCode==cnt&stmf$Sex=="b"&stmf$Year==2019),11:15])/52
    mx.2020b <- ((D.nodate.20+D.y.20)/D.y.20)*colSums(stmf[which(stmf$CountryCode==cnt&stmf$Sex=="b"&stmf$Year==2020&stmf$Week%in%(10:(10+53-Av.Weeks.21))),11:15])/max((53-Av.Weeks.21),1)
    mh1 <- (Av.Weeks*mx.2020 + (53-Av.Weeks)*mx.2019)/53
    mh2 <- (Av.Weeks*mx.2020 + (53-Av.Weeks)*mx.2020b)/53
    mh1.21 <- (Av.Weeks.21*mx.2021 + (52-Av.Weeks.21)*mx.2019)/52
    mh2.21 <- (Av.Weeks.21*mx.2021 + (52-Av.Weeks.21)*mx.2020b)/52
    e0.1 <- get.lt(mx=mh1,age=age,sex="M",hmd.ax)$ex[1]
    e0.2 <- get.lt(mx=mh2,age=age,sex="M",hmd.ax)$ex[1]
    e0.1.21 <- get.lt(mx=mh1.21,age=age,sex="M",hmd.ax)$ex[1]
    e0.2.21 <- get.lt(mx=mh2.21,age=age,sex="M",hmd.ax)$ex[1]
    line <- c(cnt,2020,e0.1,e0.2,Av.Weeks,NA,NA,NA,NA,NA,NA)
    line2 <- c(cnt,2021,e0.1.21,e0.2.21,Av.Weeks.21,NA,NA,NA,NA,NA,NA)
    mymatrix <- rbind(mymatrix,line,line2)
    e0.2019 <- get.lt(mx=mx2019,age=age,sex="M",hmd.ax)$ex[1]
}
mymatrix <- mymatrix[-1,]
